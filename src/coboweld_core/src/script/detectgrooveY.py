...
def find_orientation(path): 
  ...
  for i in range(path.shape[0]):
    ...
    y_axis = np.array([0.0, 1.0, 0.0]) # the negative X-axis of the camera
...
if __name__ == "__main__":
  ...
  # homeNj is when the camera is facing the Y-tube joint
  homeNj = [0.1780, -1.6413, -0.5957, -2.3748, 1.5487, 0.1790]
  ...
  while not rospy.is_shutdown():
    ...
    robot.movej(homeNj, 0.4, 0.4, wait=True)
    time.sleep(0.2)
