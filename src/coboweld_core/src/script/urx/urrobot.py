...
class URRobot(object):
    ...
    Since parsing the RT interface uses some CPU, and does not support all robots versions, it is disabled by default
    The RT interfaces is only used for the get_force related methods
    Rmq: A program sent to the robot is executed immendiatly and any running program is stopped
    """

    def __init__(self, host, use_rt=False):
        self.logger = logging.getLogger("urx")
        self.host = host
        self.csys = None # csys is coordinate system
        ...
        # precision of joint movement used to wait for move completion
    ...
    def movej(self, joints, acc=1.0, vel=2.5, wait=True, relative=False, threshold=None):
        """
        move in joint space
        """
        if relative:
    ...
    def movej_to_pose(self, tpose, acc=1.0, vel=2.5, wait=True, relative=False, threshold=None):
        if relative:
            l = self.getl()
    ...
    def movej_to_pose_list(self, pose_list, acc=0.1, vel=0.15, radius=0.01, wait=True, threshold=7.0):
        return self.movexs("movejp", pose_list, acc, vel, radius, wait, threshold=threshold)
